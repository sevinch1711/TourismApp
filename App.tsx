...
type ViewState = 'home' | 'vr-page' | 'culture' | 'planner' | 'map' | 'project';
        ...
        <div className="min-h-screen font-sans text-bukhara-text overflow-x-hidden bg-bukhara-bg selection:bg-bukhara-primary selection:text-white">
            ...
            >
                <div className="container mx-auto px-6 md:px-12 flex justify-between items-center">
                    ...
                    <div className={`hidden md:flex items-center gap-1 p-1 rounded-full transition-all duration-300 ${isScrolled ? 'bg-stone-100/50 border border-stone-200' : 'bg-white/10 backdrop-blur-md border border-white/20'}`}>
                        {[
                            ...
                            { id: 'map', label: UI_TEXT.map[lang], view: 'map' },
                        ...
                        <div className="hidden md:flex items-center gap-2">
                            {(['uz', 'ru', 'en'] as Language[]).map((l) => (
                                <button
                                    key={l}
                                    onClick={() => setLang(l)}
                                    className={`text-xs font-bold uppercase px-2 py-1 rounded transition-colors ${lang === l
                ...
                {isMobileMenuOpen && (
                    <div className="md:hidden absolute top-full left-0 w-full bg-bukhara-bg border-b border-gray-200 py-6 px-6 flex flex-col space-y-2 animate-in slide-in-from-top-2 shadow-xl">
                        ...
                        <button onClick={() => handleNavClick('map')} className="text-lg font-serif font-bold text-bukhara-primary text-left py-3 px-4 rounded-xl hover:bg-white">{UI_TEXT.map[lang]}</button>
                        ...
                        <div className="flex gap-2 pt-4 justify-center">
                            {(['uz', 'ru', 'en'] as Language[]).map((l) => (
                                <button
                                    key={l}
                                    onClick={() => setLang(l)}
                                    className={`px-4 py-2 rounded-lg text-sm font-bold uppercase ${lang === l ? 'bg-bukhara-primary text-white' : 'bg-white text-gray-500'
                                        }`}
                                >
                                    {l}
                                </button>
            ...
            <main className="animate-in fade-in duration-500">
                {currentView === 'home' ? (
                                    ...
                                    <div className="flex flex-wrap gap-3 animate-fade-up" style={{ animationDelay: '0.2s' }}>
                                        {filters.map((filter) => (
                                            <button
                                                ...
                                                className={`px-5 py-2.5 rounded-full text-sm font-medium transition-all duration-300 ${activeFilter === filter.id
                    ...
                    /* Map View */
                    <div className="pt-24 bg-stone-100 min-h-screen">
                        <InteractiveMap lang={lang} onNavigateDetail={handleReadMore} />
                    </div>
                ) : (
                    /* Planner View */
                    <div className="pt-24 bg-bukhara-bg">
